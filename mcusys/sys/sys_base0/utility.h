...
//全局变量
	typedef struct{
		volatile unsigned long cnt_1ms;
		volatile unsigned long cnt_1ms_H;
		unsigned long delay_k;
		DATA_POOL_T data_pool;
		FUNC_LTB_T func_ltb_tick;
		unsigned long g_rnd0;
	}GL_VAR_T;
	
#ifdef UTILITY_MAIN	
	GL_VAR_T g_var __attribute__((section(".ARM.__at_0x20000000"))) = {0,0,0,DATA_POOL_SIZE};	
#else
	extern GL_VAR_T g_var;
...
#define LMSZ(A)	(left_ms(&A)==0)
#define LMSS(A,B)	left_ms_set(&A,B)
#define LMSZPTM	(left_ms(&p->tm)==0)
#define LMSSPTM(B)	left_ms_set(&p->tm,B)
...
//-----------------------------------------------------------------------------------
//extern void * DataPool_Get(unsigned int size);	
typedef enum	
{
	DPGA_BYTE=1,
	DPGA_HWORD=2,
...
extern void * DataPool_Get_Align(unsigned int size ,DPGA_E align);
#define DataPool_Get(A) DataPool_Get_Align(A,DPGA_BYTE)
//-----------------------------------------------------------------------------------
